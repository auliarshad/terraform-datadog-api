...
module "monitor_latency_p95" {
  ...
  query              = "avg(last_1m):avg:xxt.res.ltcy.p95{cluster:${var.cluster}, environment:${var.environment}} by {host,classname,methodname} >= ${var.latency_p95_thresholds["critical"]}"
...
module "monitor_exception" {
  ...
  query              = "avg(${var.monitor_exception_time_evaluation}):sum:xxt.res.exc.count{cluster:${var.cluster}, environment:${var.environment}} by {host,classname,methodname} >= ${var.exception_thresholds["critical"]}"
